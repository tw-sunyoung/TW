...
```
글 제목: 인증서 톺아보기 시리즈 1: 인증서란? 디지털 신뢰의 시작
소속 부서: 배포플랫폼개발팀
작성자 이름: 김창현
...
여러분은 '인증서'라고 하면 어떤 생각이 떠오르시나요? 아마 많은 분들이 <strong>'매년 갱신해야 하는 귀찮은 일'</strong>을 떠올리실 겁니다. 그런데 정작 인증서가 무엇인지, 왜 필요한지는 잘 모르는 경우가 많습니다. 오늘은 인증서의 기본 개념부터 도메인 인증서 관리 실무에서 알아두면 좋은 내용까지 한번 살펴보겠습니다.

## 인증서(Certificate)란?

인증서는 **디지털 신원 증명서**입니다. 현실 세계의 신분증이 사람을 증명하듯, 디지털 세계에서는 이 인증서라는 전자 문서가 서버, 클라이언트, 또는 개인의 신원을 증명합니다.

가장 흔하게 접하는 것은 <strong>웹 서버 인증서(도메인 인증서)</strong>입니다. 브라우저 주소창의 자물쇠 아이콘을 클릭해 보면 인증서를 확인할 수 있는데, 이는 <strong>"이 서버는 진짜 example.com이 맞습니다"</strong>라고 증명하는 역할을 합니다.
...
* 두 가지 모두 PEM 또는 DER 인코딩을 사용할 수 있음
...
다음 편에서는 **짧아지는 인증서 유효 기간과 이에 대한 해결책인 ACME 프로토콜**에 대해 알아보겠습니다.

2029년 최대 47일까지 단축되는 유효 기간에 대응하는 자동화 전략과, Let's Encrypt의 무료 인증서 제공 원리를 다룹니다.
...
* Sharon Boeyen 외, RFC - Proposed Standard, 2008. 5., https://tools.ietf.org/html/rfc5280
* Qualys SSL Labs, SSL Server Test, https://www.ssllabs.com/ssltest/
* 한국전자인증, [중요공지] SSL/TLS 인증서 유효기간 단축 확정, 2025. 4. 18., https://cert.crosscert.com/%EC%A4%91%EC%9A%94%EA%B3%B5%EC%A7%80-ssl-tls-%EC%9D%B8%EC%A6%9D%EC%84%9C-%EC%9C%A0%ED%9A%A8%EA%B8%B0%EA%B0%84-%EB%8B%A8%EC%B6%95-%ED%99%95%EC%A0%95/
...
* **[Private CA](https://www.nhncloud.com/kr/service/management/private-ca)**: 조직 내·외부에서 사용하는 사설 인증서를 발급부터 폐기까지 자동화 및 표준화하는 서비스
