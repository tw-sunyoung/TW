...
4. **원격 전송**을 **사용**으로 변경하세요. **AppKey** 입력 필드에 Log & Crash Search의 Appkey를 입력한 뒤 **검증**을 클릭하세요.
...
- **인터페이스 분리**: 이 가이드에서는 출발지·목적지 인터페이스를 모두 `TRAFFIC`으로 지정했지만, **인터페이스** 탭에서 Web/WAS/DB 등 용도별 인터페이스를 추가 등록하면 인터페이스 단위로 더 세분화된 정책을 구성할 수 있습니다.
