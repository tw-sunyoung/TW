...
**정책**은 Network Firewall에서 트래픽을 제어하는 단위입니다. 출발지/목적지 인터페이스, 출발지, 목적지, 목적지 포트, 동작(허용 또는 차단)으로 구성된 ACL로, 조건에 맞는 트래픽을 허용하거나 차단하는 규칙입니다. 여기에서는 Network Firewall 정책에서 사용할 객체를 등록하고, 허용 및 차단 정책을 추가한 뒤, 트래픽 로그를 Log & Crash Search로 전송하는 시나리오를 살펴봅니다.
...
3. **추가**를 클릭한 뒤 다음 정보를 입력해 웹 서브넷 객체를 등록합니다.
    ...
    | IP 주소 | `192.168.20.0/24` | CIDR 형식으로 입력합니다. CIDR에 대한 자세한 내용은 하단의 [용어 정리](#용어-정리)를 참고하세요. |
...
2. **추가**를 클릭한 뒤 **정책 추가** 모달 창에서 다음 정보를 입력합니다.
...
3. **출발지 인터페이스** 영역에서 **추가**를 클릭하면 등록된 인터페이스 목록이 나타납니다. `TRAFFIC` 인터페이스를 선택한 뒤 **확인**을 클릭하세요.

4. **목적지 인터페이스** 영역에서 **추가**를 클릭한 뒤, 마찬가지로 `TRAFFIC` 인터페이스를 선택하세요.
...
5. **출발지** 영역에서 **추가**를 클릭하면 등록된 IP 객체 목록이 나타납니다. `web-subnet` 객체를 선택한 뒤 **확인**을 클릭하세요.

6. **목적지** 영역에서 **추가**를 클릭한 뒤, `db-subnet` 객체를 선택하세요.

7. **목적지 포트** 영역에서 **추가**를 클릭한 뒤, `mysql-port` 객체를 선택하세요.
...
2. **옵션** 탭을 클릭한 뒤 **로그 설정** 영역에서 **로그 원격 전송 설정**의 **설정 변경**을 클릭하세요. 설정 화면이 나타납니다.

3. 설정 화면 상단의 **Log & Crash Search**를 클릭하세요.

4. **원격 전송**을 **사용**으로 변경한 뒤 **저장**을 클릭하세요.
...
2. 접속이 성공하면 정책이 정상 적용된 것입니다. **로그** 탭 > **트래픽** 하위 탭에서 해당 트래픽이 허용 동작으로 기록되었는지 확인하세요. 반대로, 허용 정책에 해당하지 않는 트래픽(예: SSH 접속)을 시도하면 `default-deny` 정책에 의해 차단됩니다. 기본 차단 정책 로그를 활성화했다면 차단 로그도 확인할 수 있습니다.
...
- **인터페이스 분리**: 이 가이드에서는 출발지·목적지 인터페이스를 모두 `TRAFFIC`으로 지정했지만, **인터페이스** 탭에서 웹/WAS/DB 등 용도별 인터페이스를 추가 등록하면 인터페이스 단위로 더 세분화된 정책을 구성할 수 있습니다.
